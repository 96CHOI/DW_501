/*
    ...
    1. 귀속여부
    2.  함수는 독립적인 코드
    3. 메서드는 귀속되어서 사용되는 코드

    함수 : 코드를 모듈화 한것으로, 재사용성, 유지보수 효율성이 좋아지는 특징을 가짐
    -> 모듈에는 이름을 지정해야 하며, 모듈 삽입하게 되는 사항과 모듈이 사용 되었을때 출력되는
    사항을 확인하는 것이 필요.

    특징 : 관리하기 편함, 여러 HTML에 사용 가능.

    // input태그로부터 데이터 입력받아서

    1. 데이터가 유효한 데이터인지 검사하고 저장 하는 위치에 공간 확보가 되었는지 확인하고
    파일 저장하기 위해
    2. 파일이 생성 되었는지 확인하고
    3. 생성되었다면 파일에 저장한다.
    ...
    선언부 및 정의부
    -> 함수모듈의 출력 형태와 이름, 삽입되는 사항과 코드블럭이 추가되어 있는 부분
    ...
    보관 후 실행함수로 제어를 이동하여 초기와 -> 함수실행이 완료되면
    초기화된 변수들은 메모리 영역에서 삭제

    지역변수는 코드블럭에서만 사용가능
        코드블럭 실행이 생성되고 코드블럭 종료 시 삭제
    ...
    함수간 통신을 위한 방법 : 데이터  삽입부와 출력부를 이용해서 통신 데이터 삽입
    ( 인수 (파라미터, 매개변수) )
...
*/
/*
var res = total(5, 20); // 원하는 숫자 범위의 총 합 구하기, 외부에서 내부로 : 매개변수
...
function input_num() { // 숫자입력 받기 위한 함수
    // html에서 데이터 받기
    // 
}
